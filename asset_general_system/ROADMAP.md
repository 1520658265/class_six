...
- [ ] 地图 prompt 语义抽取升级：不要继续只靠硬编码关键词补元素。保留现有专用规则和 `custom_object` 兜底，但真实模式应接入 Gemini/LLM 做结构化对象抽取，输出区域、对象名称、数量、尺寸/footprint、是否阻挡、建议放置区域、原始描述片段等字段。
- [ ] Python CLI 增加结构化确认步骤：生成地图前先展示识别到的区域和对象，让用户逐项确认、修改、删除或补充；确认后再生成 `RPGMapSpec`、`map_data.json`、Tiled object layer 和 mock/真实预览。
- [ ] 建立 RPG 素材生产规范：按 `docs/rpg_asset_generation_standard.md` 落地 `AssetGenerationSpec`，生成前必须确认素材类型、地图位置、相对关系、footprint、源图尺寸、锚点、朝向、必备结构和禁止项。
- [ ] 建立素材验收与入库状态：公共资源库增加 `accepted / needs_review / rejected`，只有通过技术、结构、美术、地图适配验收的素材才能默认复用。
- [ ] 墙面/文字类素材拆分策略：标语、招牌、海报先生成底板或覆盖层，文字由程序后绘制；同时记录 `parent_object_id / attach_side / attach_offset / layer_order`，避免墙面元素被生成成地面物件。
