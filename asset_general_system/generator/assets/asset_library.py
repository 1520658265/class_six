...
class AssetLibrary:
    ...
    SYNONYMS = {
        ...
        "house": ["house", "hut", "cottage", "dwelling"],
    ...
    def search_by_tags(
        ...
        """
        search_groups = self._tag_search_groups(tags, use_synonyms)
        ...
        for asset in self.assets.values():
            ...
            if match_all:
                # Every requested concept must match at least one asset tag.
                if all(self._matches_any_option(asset_tags, group) for group in search_groups):
                    results.append(asset)
            else:
                # Any requested concept may match.
                if any(self._matches_any_option(asset_tags, group) for group in search_groups):
                    results.append(asset)

        # Sort by relevance (number of matching tags)
        def relevance_score(asset: AssetMetadata) -> int:
            asset_tags = [t.lower() for t in asset.tags]
            return sum(1 for group in search_groups if self._matches_any_option(asset_tags, group))
    ...
    def find_missing_assets(
        ...
        for tag in required_tags:
            matches = self.search_by_tags([tag], use_synonyms=False)
            if not matches:
                missing.append(tag)
        return missing

    def _tag_search_groups(self, tags: list[str], use_synonyms: bool) -> list[set[str]]:
        """
        Convert each query tag into a group of acceptable terms.

        Keeping one group per input tag preserves match_all semantics: searching
        for ["tree", "forest"] means "tree-like AND forest", not every synonym
        of tree plus forest at the same time.
        """
        groups: list[set[str]] = []
        for tag in tags:
            tag_lower = tag.lower()
            ...
            if use_synonyms:
                for synonyms in self.SYNONYMS.values():
                    lowered = {item.lower() for item in synonyms}
                    if tag_lower in lowered:
                        group.update(lowered)
            groups.append(group)
        return groups

    def _matches_any_option(self, asset_tags: list[str], options: set[str]) -> bool:
        return any(option in asset_tag for option in options for asset_tag in asset_tags)
