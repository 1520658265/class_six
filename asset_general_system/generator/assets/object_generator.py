...
class ObjectGenerator:
    ...
    def _build_prompt(self, request: ObjectGenerationRequest) -> str:
        ...
        if request.style == ImageStyle.PIXEL_ART:
            ...
            parts.append("32x32 or 64x64 retro game sprite")
            parts.append("clean pixel edges")
        elif request.style == ImageStyle.HAND_DRAWN:
            parts.append("hand-drawn illustration style")
            parts.append("2D game asset")

        parts.append("transparent background")
        parts.append("top-down or isometric view")
        parts.append("no shadows on ground")
        parts.append("centered in frame")

        if request.theme:
            parts.append(f"themed for {', '.join(request.theme)}")
