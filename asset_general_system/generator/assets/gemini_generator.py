...
class GeminiImageGenerator(ImageGenerator):
                ...
                for part in parts:
                    ...
                    if inline:
                        ...
                        if request.transparency == TransparencyMode.REQUIRED:
                            output_path = self._ensure_transparency(output_path, request.size)
    ...
    def _build_prompt(self, request: ImageGenerationRequest) -> str:
        ...
        if request.style == ImageStyle.PIXEL_ART:
            parts.append("pixel art style, 8-bit retro game graphics")
        elif request.style == ImageStyle.HAND_DRAWN:
            parts.append("hand-drawn illustration style")
        elif request.style == ImageStyle.LOW_POLY:
            parts.append("low-poly 3D style")

        # 透明背景
        if request.transparency == TransparencyMode.REQUIRED:
            parts.append("transparent background, PNG with alpha channel")
            parts.append("isolated object, no background elements")

        # tile 对齐（用于 tileset）
        if request.tile_aligned and request.tile_size:
            parts.append(f"tile-aligned, fits {request.tile_size[0]}x{request.tile_size[1]} grid")
            parts.append("seamless edges, no overlapping")

        # 通用约束
        parts.append("clean, professional game asset")
        parts.append("no text, no watermarks")
    ...
    def _ensure_transparency(self, image_path: Path, target_size: tuple[int, int]) -> Path:
        """
        后处理：确保透明背景。
