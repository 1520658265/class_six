...
def test_character_generator_8_direction_walk_sheet():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        request = CharacterGenerationRequest(
            character_type="rabbit_warrior",
            description="armored rabbit warrior walk sheet",
            frame_size=(48, 64),
        ...
        metadata = result.metadata
        assert [d.value for d in metadata.directions] == [d.value for d in directions]
        assert len(metadata.animations) == 8
        assert metadata.animations["walk_down_left"].direction == Direction.DOWN_LEFT
        assert metadata.animations["walk_up_right"].direction == Direction.UP_RIGHT
        assert metadata.animations["walk_down"].fps == 10
